    ...
    class WriteGate(h.Gate):
        ...
        def probes(self, ctx):
            ...
            for desc, call, want in (
                    ...
                    # **인라인 코드는 우리 영역을 언급할 때만 묻는다.** 예전에는
                    # 인라인이기만 하면 물었고, 그 대가로 `print(1+1)` 까지 사람을
                    # 세웠다. 이제 이 규칙의 고유 기여는 **경로를 쪼개 조립하는**
                    # 경우 하나이므로, 그 모양을 그대로 탐침으로 둔다 —
                    # `floor_named` 는 이것을 못 잡는다(원문에 온전한 이름이 없다).
                    (h.t("인라인 코드가 바닥값을 조립해도 묻는다"),
                     h._bind(bh, 'python3 -c "open(\'.claude/harn\'+\'ess/x\',\'w\')"'),
                     True),
                    # 통과하는 쪽 — 이것이 없으면 "묻는다" 탐침만으로는 규칙을
                    # 무조건 참으로 바꿔도 초록이다. 그리고 이 자리가 곧 마찰이다.
                    (h.t("평범한 인라인 스크립트는 묻지 않는다"),
                     h._bind(bh, 'python3 -c "print(1+1)"'), False),

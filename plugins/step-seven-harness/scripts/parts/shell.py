...
def bash_unresolved(cfg, cmd):
    ...
    for seg in BASH_SPLIT.split(strip_quoted_heredocs(cmd)):
        ...
        # 이었다(126건 중 67건). 걸린 것은 거의 전부 이 모양이다:
        #
        #     SP=/private/tmp/claude-501/…/scratchpad/x.mjs
        #     cd <repo> ⏎ SP=/private/tmp/…
        #
        # 긴 임시 경로를 변수로 빼는 습관이다. 그 명령이 실제로 쓰는 곳은 저장소
        # **밖**(`/tmp`)이고, 하네스와 아무 상관이 없다.
        #
        # 그리고 이건 정확해질 수 없다 — `$SP` 가 무엇인지는 실행해 봐야 안다.
        # 좁힐 여지도 없다: "변수를 썼다" 는 사실만으로는 위험을 가릴 방법이 없다.
        # **정확하지 않은 통제는 제거한다**(POLICY 10).
        #
        # 잃는 것: 바닥값 경로가 **변수로만** 전달되는 경우
        # (`D=.claude/harness; rm $D/bin/harness`). 원문에 이름이 그대로 있으면
        # `floor_named` 가 계속 잡는다 — 위 예도 `.claude/harness` 가 원문에
        # 보이므로 잡힌다. 완전히 조립하는 경우만 놓치고, 그건 실수가 아니라
        # 작정한 우회이며 README 가 범위 밖이라고 선언한 상대다.
    return None
